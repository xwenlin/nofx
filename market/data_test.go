...
func TestCalculateIntradaySeries_VolumeCollection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(data.Volumes) != tt.expectedVolLen {
				t.Errorf("Volume length = %d, want %d", len(data.Volumes), tt.expectedVolLen)
			}
			...
			if len(data.Volumes) > 0 {
				...
				if data.Volumes[0] != expectedFirstVolume {
					t.Errorf("First volume = %.2f, want %.2f", data.Volumes[0], expectedFirstVolume)
				}

				// 验证最后一个 Volume 值
				expectedLastVolume := klines[tt.klineCount-1].Volume
				lastVolume := data.Volumes[len(data.Volumes)-1]
...
func TestCalculateIntradaySeries_VolumeValues(t *testing.T) {
	...
	if len(data.Volumes) != len(expectedVolumes) {
		t.Fatalf("Volume length = %d, want %d", len(data.Volumes), len(expectedVolumes))
	}

	for i, expected := range expectedVolumes {
		if data.Volumes[i] != expected {
			t.Errorf("Volume[%d] = %.2f, want %.2f", i, data.Volumes[i], expected)
		}
	}
}
